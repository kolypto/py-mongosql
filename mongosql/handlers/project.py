...
class MongoProject(MongoQueryHandlerBase):
    ...
    def input(self, projection):
        ...
        if self.mode == self.MODE_EXCLUDE and self.default_exclude:
            ...
            self.projection.update({k: 0 for k in self.default_exclude})
    ...
    def compile_options(self, as_relation):
        ...
        # (by default, with input(None), mode=exclude, and projection={}
        if self.mode == self.MODE_EXCLUDE and not self.projection:
    ...
    def merge(self, projection):
        ...
        """
        # Validate
        mode, projection = self._input_validate(projection)
        ...
        if self.mode == mode:
            ...
            # this is a short-cut
            drop_keys = set(projection.keys()) & set(self.projection.keys())
            for k in drop_keys:
